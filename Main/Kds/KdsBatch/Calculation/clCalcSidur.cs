...
namespace KdsBatch
{
    class clCalcSidur : GlobalDataset
    {
        ...
        public void CalcRechiv7()
        {
            ...
            try
            {
                if (_oGeneralData.objMeafyeneyOved.iMeafyen31 == 1)
                {
                    ...
                    for (int I = 0; I < drSidurim.Length; I++)
                    {
                        ...
                        if (iMisparSidur != 99006)
                        {
                            ...
                            if (CheckSidurBySectorAvoda(ref drSidurim[I], iMisparSidur, clGeneral.enSectorAvoda.Tafkid.GetHashCode()) && iHashlama == 0)
                            {
                                ...
                                if ((dShatHatchalaLetashlum <= _oGeneralData.objParameters.dEndAruchatTzaharayim) && (dShatGmarLetashlum >= _oGeneralData.objParameters.dStartAruchatTzaharayim))
                                {
                                    if ((dShatHatchalaLetashlum <= _oGeneralData.objParameters.dStartAruchatTzaharayim) && (dShatGmarLetashlum >= _oGeneralData.objParameters.dStartAruchatTzaharayim))
                                    {
                                        fTempX = float.Parse((dShatGmarLetashlum - _oGeneralData.objParameters.dStartAruchatTzaharayim).TotalMinutes.ToString());
                                    }
                                    else if ((dShatHatchalaLetashlum > _oGeneralData.objParameters.dStartAruchatTzaharayim) && (dShatHatchalaLetashlum < _oGeneralData.objParameters.dEndAruchatTzaharayim) && (dShatGmarLetashlum > _oGeneralData.objParameters.dEndAruchatTzaharayim))
                                    {
                                        fTempX = float.Parse((_oGeneralData.objParameters.dEndAruchatTzaharayim - dShatHatchalaLetashlum).TotalMinutes.ToString());
                                    }
                                    else if ((dShatHatchalaLetashlum > _oGeneralData.objParameters.dStartAruchatTzaharayim) && (dShatGmarLetashlum < _oGeneralData.objParameters.dEndAruchatTzaharayim))
                                    {
        ...
        public void CalcRechiv131()
        {
            ...
            try
            {
                ...
                {
                    _drSidurim = clCalcData.DtYemeyAvoda.Select("Lo_letashlum=0 and mispar_sidur is not null and taarich=Convert('" + dTaarich.ToShortDateString() + "', 'System.DateTime') and MISPAR_SIDUR IN(99703,99202,99701,99014,99010 ,99006)");
                    for (int I = 0; I < _drSidurim.Length; I++)
                    {
                        iMisparSidur = int.Parse(_drSidurim[I]["mispar_sidur"].ToString());
                        fErech = 0;
                ...
                if (clDefinitions.CheckShaaton(clCalcData.DtSugeyYamimMeyuchadim, iSugYom, dTaarich))
                {
                    _drSidurim = clCalcData.DtYemeyAvoda.Select("Lo_letashlum=0 and mispar_sidur is not null and taarich=Convert('" + dTaarich.ToShortDateString() + "', 'System.DateTime') and MISPAR_SIDUR IN(99012,99013)");
                    for (int I = 0; I < _drSidurim.Length; I++)
                    {
                        iMisparSidur = int.Parse(_drSidurim[I]["mispar_sidur"].ToString());
                        fErech = 0;
                ...
                if (_drSidurRagil.Length > 0)
                {
                    for (int I = 0; I < _drSidurRagil.Length; I++)
                    {
                        ...
                        if (bYeshSidur)
                        {
                            ...
                            iSugYomNext = clDefinitions.GetSugYom(clCalcData.DtYamimMeyuchadim, DateTime.Parse(dTaarich.ToShortDateString()).AddDays(1), clCalcData.DtSugeyYamimMeyuchadim, _oGeneralData.objMeafyeneyOved.iMeafyen56);
                 ...
                 for (int I = 0; I < _drSidurim.Length; I++)
                 {
                     ...
                     if (!clDefinitions.CheckShaaton(clCalcData.DtSugeyYamimMeyuchadim, clCalcData.iSugYom, dTaarich))
                     {
                             ...
                             if (iMisparSidur.ToString().Substring(0, 2) != "99")
                             {
                                 ...
                                 if (bYeshSidur)
                                 {
                                     fErech = clCalcData.GetSumErechRechiv(dtPeiluyot.Compute("sum(zmanElement)", "SUBSTRING(makat_nesia,1,3)='730'"));
                                     oPeilut.CalcZmaneyAruchot(dtPeiluyot, 730, dShatHatchalaLetaslum, dShatGmarLetashlum, out  fZmanAruchatBokerSidur, out  fZmanAruchatTzharimSidur, out fZmanAruchatErevSidur);
                     ...
                     if (!clDefinitions.CheckShaaton(clCalcData.DtSugeyYamimMeyuchadim, clCalcData.iSugYom, dTaarich))
                     {
                         ...
                         else
                         {
                             ...
                             if (iMisparSidur.ToString().Substring(0, 2) != "99")
                             {
                                 ...
                                 if (bYeshSidur)
                                 {
                                     fErech = clCalcData.GetSumErechRechiv(dtPeiluyot.Compute("sum(zmanElement)", "SUBSTRING(makat_nesia,1,3)='750'"));
                                     oPeilut.CalcZmaneyAruchot(dtPeiluyot, 750, dShatHatchalaLetaslum, dShatGmarLetashlum, out  fZmanAruchatBokerSidur, out  fZmanAruchatTzharimSidur, out fZmanAruchatErevSidur);
    ...
    private bool CheckShabat(int iMisparSidur,int iDay,int iSugYom,DateTime dShatHatchalaSidur,DateTime dShatGmarSidur,ref float fErechChol,ref float fErechShabat,bool bCheckMafil)
     {
        ...
        if (bShabat)
        {
           ...
           dNextDay = dTaarich.AddDays(1);
            ...
            { bNextShabat = true; }
            else { bNextShabat = clDefinitions.CheckShaaton(clCalcData.DtSugeyYamimMeyuchadim, clDefinitions.GetSugYom(clCalcData.DtYamimMeyuchadim, dNextDay, clCalcData.DtSugeyYamimMeyuchadim, _oGeneralData.objMeafyeneyOved.iMeafyen56), dNextDay); }
