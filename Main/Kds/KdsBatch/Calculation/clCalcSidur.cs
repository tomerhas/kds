    ...
    private bool CheckShabat(int iMisparSidur,int iDay,int iSugYom,DateTime dShatHatchalaSidur,DateTime dShatGmarSidur,ref float fErechChol,ref float fErechShabat,bool bCheckMafil)
     {
        ...
        if (bShabat)
        {
           ...
           dNextDay = dTaarich.AddDays(1);
            ...
            { bNextShabat = true; }
            else { bNextShabat = clDefinitions.CheckShaaton(clCalcData.DtSugeyYamimMeyuchadim, clGeneral.GetSugYom(clCalcData.DtYamimMeyuchadim, dNextDay, clCalcData.DtSugeyYamimMeyuchadim, _oGeneralData.objMeafyeneyOved.iMeafyen56), dNextDay); }
