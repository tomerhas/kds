    ...
    class CalcDay
    {
      ...
      //  public int SugYom { get; set; }
        ...
        private void CalcRechiv296()
        {
            ...
            {
                clLogBakashot.SetError(objOved.iBakashaId, objOved.Mispar_ishi, "E", clGeneral.enRechivim.ZmanElementZarNahagut.GetHashCode(), objOved.Taarich, "CalcDay: " + ex.StackTrace + "\n message: " + ex.Message);
                throw (ex);
            }
               ...
               if (!(objOved.objPirteyOved.iDirug == 85 && objOved.objPirteyOved.iDarga == 30))
                {
                    ...
                    if (fMichsaMechushevet > 0)
                    {
                        ...
                        {
                            if (objOved.Taarich < DateTime.Parse("01/01/2015"))
                            {
                            ...
                            {
                                frechiv30 = oCalcBL.GetSumErechRechiv(ListOfSum, clGeneral.enRechivim.DakotPremiaYomit); //oCalcBL.GetSumErechRechiv(objOved._dsChishuv.Tables["CHISHUV_YOM"], clGeneral.enRechivim.DakotPremiaYomit.GetHashCode(), objOved.Taarich);
                                fRechiv28 = oCalcBL.GetSumErechRechiv(ListOfSum, clGeneral.enRechivim.DakotPremiaVisa); //oCalcBL.GetSumErechRechiv(objOved._dsChishuv.Tables["CHISHUV_YOM"], clGeneral.enRechivim.DakotPremiaVisa.GetHashCode(), objOved.Taarich);

                                fSumDakotRechiv = ((frechiv30 + fRechiv28) / fDakotNochehut) * fMichsaMechushevet;
                            }
                        }
                        else
                        {
        ...
        private int GetDakotHafchataMizmanNesia()
        {
            try
            {
                ...
                if (objOved.Taarich >= objOved.objParameters.dTaarichHafalatHafchataZmanNesia)
                    if (!String.IsNullOrEmpty(objOved.objMeafyeneyOved.sMeafyen51))
        ...
        private void CalcRechiv293()
        {
            ...
            {
                clLogBakashot.SetError(objOved.iBakashaId, objOved.Mispar_ishi, "E", clGeneral.enRechivim.YemeyNochechut.GetHashCode(), objOved.Taarich, "CalcDay: " + ex.StackTrace + "\n message: " + ex.Message);
                throw (ex);
            }
        ...
        private void CalcRechiv295()
        {
            ...
            {
                clLogBakashot.SetError(objOved.iBakashaId, objOved.Mispar_ishi, "E", clGeneral.enRechivim.NesiaBerechevMuganET.GetHashCode(), objOved.Taarich, "CalcDay: " + ex.StackTrace + "\n message: " + ex.Message);
                throw (ex);
            }
        }
            ...
            try
            {
                ...
                while (dTarMe <= dTarAd)
                {
                    ...
                    if (fMichsaYomit > 0 && fDakotNochehut == 0 && fChofesh == 0 && fHeadrut == 0 && objOved.DtYemeyAvoda.Select("taarich=Convert('" + dTarMe.ToShortDateString() + "', 'System.DateTime') and Lo_letashlum=0 and mispar_sidur is not null").Length == 0)
                    {
                        ...
                        if (objOved.objMeafyeneyOved.iMeafyen33 == 0)
                        {
