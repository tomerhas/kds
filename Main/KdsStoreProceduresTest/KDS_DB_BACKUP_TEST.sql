...
BEGIN  
      DELETE FROM TB_RIKUZ_PDF R 
       where exists (select *   from  TB_OVDIM_PROCESSES p
...
PROCEDURE pro_prepare_netunim_lechishuv(p_tar_me IN DATE,p_tar_ad IN DATE,
 ...
 v_ad_taarich:=to_date('30/09/2010','dd/mm/yyyy');
...
END Pkg_Ovdim;
/
CREATE OR REPLACE PROCEDURE pro_ins_amount_of_drivers_tnua(p_bakasha_id number) IS
    rc NUMBER;
    CURSOR v_Cur IS
 /* Formatted on 24/10/2012 1:04:16 PM (QP5 v5.163.1008.3004) */
WITH bm
     AS (SELECT TO_DATE ('01/' || p.erech, 'dd/mm/yyyy') Bakasha_month,
                ADD_MONTHS (TO_DATE ('01/' || p.erech, 'dd/mm/yyyy'), -7)
                   tar_hatchala,
                LAST_DAY (
                   ADD_MONTHS (TO_DATE ('01/' || p.erech, 'dd/mm/yyyy'), -1))
                   tar_siyum
           FROM tb_bakashot_params p
          WHERE p.param_id = 2 AND p.bakasha_id = p_bakasha_id),
     bb
     AS (SELECT DISTINCT
                co.mispar_ishi,
                co.Taarich,
                MAX (B.bakasha_id)
                   OVER (PARTITION BY co.mispar_ishi, co.Taarich)
                   bakasha_id
           FROM TB_CHISHUV_CHODESH_OVDIM co, TB_BAKASHOT b, bm
          WHERE     co.taarich BETWEEN tar_hatchala AND tar_siyum
                AND B.BAKASHA_ID = CO.BAKASHA_ID
                AND b.Huavra_Lesachar = 1),
     pp
     AS (SELECT DISTINCT p.mispar_ishi,
                         p.me_tarich,
                         p.ad_tarich,
                         p.gil,
                         p.maamad,
           ...
           FROM TB_CHISHUV_chodesh_OVDIM c,
                ...
                bm,
                bb
          WHERE     c.taarich BETWEEN tar_hatchala AND tar_siyum
                AND c.bakasha_id = bb.bakasha_id
                AND c.mispar_ishi = bb.mispar_ishi
                AND c.taarich = bb.taarich
                AND (c.taarich BETWEEN p.ME_TARICH AND  p.ad_TARICH
                     OR (ADD_MONTHS (c.taarich, 1) - 1) BETWEEN p.ME_TARICH  AND p.ad_TARICH
                     OR p.ME_TARICH >= c.taarich AND p.ad_TARICH<= (ADD_MONTHS (c.taarich, 1) - 1))
                AND c.mispar_ishi = p.mispar_ishi
                AND p.gil IN (0, 1, 2)),
     ss
     AS (  SELECT c.taarich,
                  c.mispar_ishi,
                  SUM (c.erech_rechiv) erech_rechiv,
                  MAX (
                     CASE
                     ...
                     END)
                     snif_tnua
             FROM TB_CHISHUV_SIDUR_OVDIM c, bb, bm
            WHERE     c.bakasha_id = bb.bakasha_id
                  AND c.mispar_ishi = bb.mispar_ishi
                  AND c.taarich BETWEEN bm.tar_hatchala AND bm.tar_siyum
                  AND c.kod_rechiv IN (2, 35, 189)
         GROUP BY c.taarich, c.mispar_ishi)
SELECT s.tar_hatchala,
       s.tar_siyum,
       NVL (s.taarich, so.taarich) taarich,
       NVL (s.KOD_SNIF_TNUAA, so.KOD_SNIF_TNUAA) KOD_SNIF_TNUAA,
       s.count_tzair_snif,
       s.count_kashish_snif,
       s.count_kshishon_snif,
       s.count_gimlai_snif,
       round(s.shaot_tzair_snif) shaot_tzair_snif,
       round(s.shaot_kashish_snif) shaot_kashish_snif,
       round(s.shaot_kshishon_snif) shaot_kshishon_snif,
       round(s.shaot_gimlai_snif) shaot_gimlai_snif,
       round(s.shaot_meal_250_snif) shaot_meal_250_snif,
      round(so.shaot_tzair) shaot_tzair,
       round(so.shaot_kashish) shaot_kashish,
       round(so.shaot_kshishon) shaot_kshishon,
       round(so.shaot_gimlai) shaot_gimlai,
       round(so.shaot_minhal) shaot_minhal
  FROM    ( /*שליפת נתונים לסניף*/
   SELECT a.tar_hatchala,
                    a.tar_siyum,
                    a.taarich,
                    a.KOD_SNIF_TNUAA,
                    SUM (
                       CASE
                          WHEN (    a.gil = 0  AND a.maamad NOT IN (228, 238, 248, 258) AND a.shaot_tzair > 0)
                          THEN     1   ELSE  0 END) count_tzair_snif,
                    SUM (
                       CASE
                          WHEN (    a.gil = 2  AND a.maamad NOT IN (228, 238, 248, 258) AND a.shaot_kashish > 0)
                         THEN     1   ELSE  0 END)  count_kashish_snif,
                    SUM (
                       CASE
                          WHEN (    a.gil = 1 AND a.maamad NOT IN (228, 238, 248, 258) AND a.shaot_kshishon > 0)
                         THEN     1   ELSE  0 END)  count_kshishon_snif,
                    SUM (
                       CASE
                          WHEN (a.maamad IN (228, 238, 248, 258)  AND a.shaot_gimlai > 0)
                         THEN     1   ELSE  0 END) count_gimlai_snif,
                    SUM (
                       CASE
                          WHEN (    a.gil = 0  AND a.maamad NOT IN (228, 238, 248, 258) AND a.shaot_tzair > 0)
                          THEN   a.shaot_tzair ELSE  0  END) shaot_tzair_snif,
                    SUM (
                       CASE
                          WHEN (    a.gil = 2 AND a.maamad NOT IN (228, 238, 248, 258) AND a.shaot_kashish > 0)
                          THEN  a.shaot_kashish ELSE   0 END) shaot_kashish_snif,
                    SUM (
                       CASE
                          WHEN (    a.gil = 1 AND a.maamad NOT IN (228, 238, 248, 258)   AND a.shaot_kshishon > 0)
                          THEN a.shaot_kshishon  ELSE   0  END)  shaot_kshishon_snif,
                    SUM (
                       CASE
                          WHEN (a.maamad IN (228, 238, 248, 258)   AND a.shaot_gimlai > 0)
                          THEN  a.shaot_gimlai  ELSE 0  END)  shaot_gimlai_snif,
                    SUM (
                       CASE
                          WHEN (a.shaot_nahagut > 250)
                          THEN (a.shaot_nahagut - 250)   ELSE   0  END)  shaot_meal_250_snif
               FROM (  SELECT c.taarich,
                              S.KOD_SNIF_TNUAA,
                              c.mispar_ishi,
                              bm.tar_hatchala,
                              bm.tar_siyum,
                              SUM (
                                 CASE
                                    WHEN (pp.gil = 0 AND pp.maamad NOT IN  (228, 238, 248, 258))
                                    THEN  c.erech_rechiv ELSE  0 END)/ 60  shaot_tzair,
                              SUM (
                                 CASE
                                    WHEN (pp.gil = 2AND pp.maamad NOT IN (228, 238, 248, 258))
                                    THEN    c.erech_rechiv ELSE    0  END) / 60 shaot_kashish,
                              SUM (
                                 CASE
                                    WHEN (pp.gil = 1  AND pp.maamad NOT IN    (228, 238, 248, 258))
                                    THEN c.erech_rechiv ELSE  0  END) / 60 shaot_kshishon,
                              SUM (
                                 CASE
                                    WHEN (pp.maamad IN (228, 238, 248, 258))
                                    THEN  c.erech_rechiv ELSE   0 END)  / 60  shaot_gimlai,
                              SUM (c.erech_rechiv) / 60 shaot_nahagut,
                              pp.gil,
                              pp.maamad
                         FROM TB_CHISHUV_CHODESH_OVDIM c,
                              bb,
                        ...
                        WHERE c.bakasha_id = bb.bakasha_id
                              AND c.mispar_ishi = bb.mispar_ishi
                              AND c.taarich BETWEEN bm.tar_hatchala
                                                AND bm.tar_siyum
                              AND c.taarich = bb.taarich
                              AND c.taarich BETWEEN pp.me_tarich AND pp.ad_tarich
                              AND c.kod_rechiv IN (2, 35, 189)
                              AND pp.ISUK >= 500
                              AND pp.ISUK < 600
                              ...
                              AND SA.KOD_SNIF_AV = pp.SNIF_AV
                              AND SA.SNIF_TNUA = S.KOD_SNIF_TNUAA
                     GROUP BY c.taarich,
                              S.KOD_SNIF_TNUAA,
                              c.mispar_ishi,
                              pp.gil,
           ...
           GROUP BY a.taarich,
                    ...
                    a.tar_siyum) s
       FULL JOIN
          (  /* שליפת נתונים ללא שיוך סניפי*/
          SELECT e.taarich,
                    e.KOD_SNIF_TNUAA,
                     SUM (
                       CASE
                          WHEN (    e.ISUK >= 500 AND e.ISUK < 600  AND e.gil = 0
                                AND e.maamad NOT IN (228, 238, 248, 258) AND e.shaot_tzair > 0)
                          THEN  e.shaot_tzair  ELSE  0  END) shaot_tzair,
                    SUM (
                       CASE
                          WHEN (    e.ISUK >= 500 AND e.ISUK < 600
                                AND e.gil = 2 AND e.maamad NOT IN (228, 238, 248, 258) AND e.shaot_kashish > 0)
                          THEN  e.shaot_kashish ELSE  0 END)shaot_kashish,
                    SUM (
                       CASE
                          WHEN (    e.ISUK >= 500 AND e.ISUK < 600
                                AND e.gil = 1AND e.maamad NOT IN (228, 238, 248, 258)AND e.shaot_kshishon > 0)
                          THEN e.shaot_kshishon ELSE  0 END)   shaot_kshishon,
                    SUM (
                       CASE
                          WHEN e.ISUK >= 500 AND e.ISUK < 600
                               AND (e.maamad IN (228, 238, 248, 258) AND e.shaot_gimlai > 0)
                          THEN   e.shaot_gimlai ELSE    0 END)  shaot_gimlai,
                    SUM (
                       CASE
                          WHEN (e.ISUK < 500 OR e.ISUK >= 600)  AND e.maamad NOT IN (228, 238, 248, 258)
                          THEN  e.shaot_minhal ELSE   0 END) shaot_minhal
               FROM (  SELECT TRUNC (ss.taarich, 'MM') taarich,
                              st.KOD_SNIF_TNUAA,
                              pp.gil,
                              pp.maamad,
                              pp.isuk,
                              SUM (
                                 CASE
                                    WHEN (    pp.ISUK >= 500  AND pp.ISUK < 600
                                          AND pp.gil = 0 AND pp.maamad NOT IN  (228, 238, 248, 258))
                                    THEN ss.erech_rechiv ELSE   0 END) / 60   shaot_tzair,
                              SUM (
                                 CASE
                                    WHEN (    pp.ISUK >= 500  AND pp.ISUK < 600
                                          AND pp.gil = 2 AND pp.maamad NOT IN    (228, 238, 248, 258))
                                    THEN  ss.erech_rechiv ELSE  0  END) / 60   shaot_kashish,
                              SUM (
                                 CASE
                                    WHEN (    pp.ISUK >= 500AND pp.ISUK < 600
                                          AND pp.gil = 1  AND pp.maamad NOT IN (228, 238, 248, 258))
                                    THEN  ss.erech_rechiv  ELSE 0 END)/ 60 shaot_kshishon,
                              SUM (
                                 CASE
                                    WHEN (    pp.ISUK >= 500  AND pp.ISUK < 600
                                          AND pp.maamad IN (228, 238, 248, 258))
                                    THEN  ss.erech_rechiv ELSE   0 END) / 60 shaot_gimlai,
                              SUM (
                                 CASE
                                    WHEN ( (pp.ISUK < 500 OR pp.ISUK >= 600)   AND pp.maamad NOT IN  (228, 238, 248, 258))
                                    THEN  ss.erech_rechiv ELSE  0 END) / 60  shaot_minhal
                         FROM CTB_SNIFEY_TNUAA st, pp, ss
                        WHERE ss.snif_tnua = st.KOD_SNIF_TNUAA
                              AND ss.TAARICH BETWEEN pp.me_tarich
                                                 AND pp.ad_tarich
                              AND ss.mispar_ishi = pp.mispar_ishi
                     GROUP BY TRUNC (ss.taarich, 'MM'),
                              St.KOD_SNIF_TNUAA,
                              ss.mispar_ishi,
                              pp.gil,
                              pp.maamad,
                              pp.isuk) e
           GROUP BY e.taarich, e.KOD_SNIF_TNUAA) so
       ON so.taarich = s.taarich AND so.KOD_SNIF_TNUAA = s.KOD_SNIF_TNUAA;
                                           
    v_rec v_Cur%ROWTYPE;
     v_count_drivers7 number;
      v_count_yamey_chol number;
      v_first_sunday date;
      v_last_shabat date;
BEGIN
 
     /*שליפת כמות ימים, תאריכים, כמות נהגים שעבדו 7 ימים ברצף*/
          /* Formatted on 24/10/2012 2:30:21 PM (QP5 v5.163.1008.3004) */
WITH bm
     AS (SELECT TO_DATE ('01/' || p.erech, 'dd/mm/yyyy') Bakasha_month,
                ADD_MONTHS (TO_DATE ('01/' || p.erech, 'dd/mm/yyyy'), -7)
                   tar_hatchala, LAST_DAY ( ADD_MONTHS (TO_DATE ('01/' || p.erech, 'dd/mm/yyyy'), -1))
                   tar_siyum
           FROM tb_bakashot_params p
          WHERE p.param_id = 2 AND p.bakasha_id = p_bakasha_id),
     bb
     AS (SELECT DISTINCT
                ...
                MAX (B.bakasha_id)
                   ...
                   bakasha_id
           FROM TB_CHISHUV_CHODESH_OVDIM co, TB_BAKASHOT b, bm
          WHERE     co.taarich BETWEEN bm.tar_hatchala AND bm.tar_siyum
                AND B.BAKASHA_ID = CO.BAKASHA_ID
                AND b.Huavra_Lesachar = 1),
     yy
     AS (SELECT yy.taarich - 6 from_dt,
                yy.taarich to_dt,
                FIRST_VALUE (yy.taarich - 6) OVER (ORDER BY yy.taarich - 6 ASC)first_sunday,
                  FIRST_VALUE (yy.taarich) OVER (ORDER BY yy.taarich DESC)  last_shabat
           FROM (SELECT DISTINCT
                        y.taarich,
                        TO_NUMBER (TO_CHAR (y.taarich, 'D')) / 7 shabat,
                        bm.tar_hatchala
                   FROM tb_yamey_avoda_ovdim y, bm
                  WHERE y.taarich BETWEEN bm.tar_hatchala AND bm.tar_siyum) yy
          WHERE yy.shabat = 1  AND TRUNC (yy.taarich - 6, 'MM') >= yy.tar_hatchala),
     pp
     AS (SELECT DISTINCT p.mispar_ishi,
                         p.me_tarich,
                         p.ad_tarich,
          ...
          WHERE     c.taarich BETWEEN tar_hatchala AND tar_siyum
                ...
                AND c.mispar_ishi = bb.mispar_ishi
                AND c.taarich = bb.taarich
                AND (c.taarich BETWEEN p.ME_TARICH
                                   AND NVL (   p.ad_TARICH, TO_DATE ('01/01/9999',   'dd/mm/yyyy'))
                     OR (ADD_MONTHS (c.taarich, 1) - 1) BETWEEN p.ME_TARICH
                                  AND NVL (  p.ad_TARICH,  TO_DATE (  '01/01/9999',  'dd/mm/yyyy'))
                     OR p.ME_TARICH >= c.taarich
                        AND NVL (p.ad_TARICH, TO_DATE ('01/01/9999', 'dd/mm/yyyy')) <=    (ADD_MONTHS (c.taarich, 1) - 1))
                AND c.mispar_ishi = p.mispar_ishi
                AND p.gil IN (0, 1, 2)),
     cc
     AS (SELECT COUNT (*) count_yamey_chol
           FROM (SELECT DISTINCT y.taarich
                   FROM tb_yamey_avoda_ovdim y, bm
                  WHERE y.taarich BETWEEN bm.tar_hatchala AND bm.tar_siyum) y,
                (SELECT M.TAARICH,
                        M.SUG_YOM,
                        s.shbaton,
                        s.erev_shishi_chag
  ...
  SELECT first_sunday,
         last_shabat,
         COUNT (*) count_drivers,
         cc.count_yamey_chol into v_first_sunday,v_last_shabat,v_count_drivers7,v_count_yamey_chol
    FROM cc,
         (  SELECT cc.mispar_ishi,
                   yy.from_dt,
                   yy.to_dt,
                   yy.first_sunday,
                   yy.last_shabat,
              ...
              FROM yy,
                   pp,
                   (  SELECT c.taarich, c.mispar_ishi, SUM (c.erech_rechiv) erech
                        FROM TB_CHISHUV_yomi_OVDIM c, bb, bm
                       WHERE     c.bakasha_id = bb.bakasha_id
                             AND c.mispar_ishi = bb.mispar_ishi
                             AND c.kod_rechiv IN (2, 35, 189)
                             --and c.mispar_ishi=87746
                             AND c.taarich BETWEEN bm.tar_hatchala AND bm.tar_siyum
                    GROUP BY c.mispar_ishi, c.taarich) cc
             WHERE     cc.taarich BETWEEN yy.from_dt AND yy.to_dt
                   AND pp.ISUK >= 500
                   AND pp.ISUK < 600
                   AND cc.taarich BETWEEN pp.me_tarich AND pp.ad_tarich
                   AND cc.mispar_ishi = pp.mispar_ishi
                   AND cc.erech / 60 > 0
          GROUP BY cc.mispar_ishi,
                   yy.from_dt,
                   yy.to_dt,
                   yy.first_sunday,
                   yy.last_shabat
            HAVING COUNT (*) = 7)
       GROUP BY first_sunday, last_shabat, cc.count_yamey_chol;
          ...
          LOOP
                ...
                EXIT WHEN v_Cur%NOTFOUND;
                  begin
                   INSERT INTO tichnun.EXAMINE_AMOUNT_OF_DRIVERS_KDS@kds_gw_at_tnua
                              ( START_DT  , BRANCH       ,YOUNG_DRIVERS_HOURS , MATURE_DRIVERS_HOURS,
                              OLDER_DRIVERS_HOURS,PENSIONER_DRIVERS_HOURS, ADMIN_DRIVERS_HOURS , YOUNG_DRIVERS_TOTAL_HOURS ,
                              MATURE_DRIVERS_TOTAL_HOURS  , OLDER_DRIVERS_TOTAL_HOURS  , PENSIONER_DRIVERS_TOTAL_HOURS ,
                              MATURE_DRIVERS   , OLDER_DRIVERS, YOUNG_DRIVERS   ,  PENSIONER_DRIVERS, HOURS_ABOVE_250 ,
                              CASE_OF_FULL_WEEK_DRIVING  , NUMBER_OF_WEEK_DAYS    , SUNDAY_FROM_DATE   , SATURDAY_TO_DATE   )
                      VALUES (v_rec.taarich,v_rec.KOD_SNIF_TNUAA, v_rec.shaot_tzair, v_rec.shaot_kshishon,
                          v_rec.shaot_kashish,  v_rec.shaot_gimlai, v_rec.shaot_minhal, v_rec.shaot_tzair_snif,
                        v_rec.shaot_kshishon_snif,  v_rec.shaot_kashish_snif, v_rec.shaot_gimlai_snif, 
                          v_rec.count_kshishon_snif,v_rec.count_kashish_snif,  v_rec.count_tzair_snif,  v_rec.count_gimlai_snif,v_rec.shaot_meal_250_snif,
                        v_count_drivers7, v_count_yamey_chol, v_first_sunday,v_last_shabat);
                       EXCEPTION
                          WHEN DUP_VAL_ON_INDEX THEN
                              begin
                               UPDATE tichnun.EXAMINE_AMOUNT_OF_DRIVERS_KDS@kds_gw_at_tnua
                                      set YOUNG_DRIVERS_HOURS=v_rec.shaot_tzair,
                                       MATURE_DRIVERS_HOURS=v_rec.shaot_kshishon ,
                                      OLDER_DRIVERS_HOURS= v_rec.shaot_kashish,
                                      PENSIONER_DRIVERS_HOURS= v_rec.shaot_gimlai,
                                      ADMIN_DRIVERS_HOURS= v_rec.shaot_minhal ,
                                       YOUNG_DRIVERS_TOTAL_HOURS=v_rec.shaot_tzair_snif ,
                                      MATURE_DRIVERS_TOTAL_HOURS=v_rec.shaot_kshishon_snif  ,
                                       OLDER_DRIVERS_TOTAL_HOURS=  v_rec.shaot_kashish_snif , 
                                       PENSIONER_DRIVERS_TOTAL_HOURS=v_rec.shaot_gimlai_snif ,
                                      MATURE_DRIVERS=   v_rec.count_kshishon_snif ,
                                       OLDER_DRIVERS=v_rec.count_kashish_snif, 
                                       YOUNG_DRIVERS = v_rec.count_tzair_snif  ,
                                         PENSIONER_DRIVERS= v_rec.count_gimlai_snif,
                                          HOURS_ABOVE_250=v_rec.shaot_meal_250_snif ,
                                      CASE_OF_FULL_WEEK_DRIVING=v_count_drivers7  , 
                                      NUMBER_OF_WEEK_DAYS =v_count_yamey_chol   , 
                                      SUNDAY_FROM_DATE =v_first_sunday  ,
                                       SATURDAY_TO_DATE= v_last_shabat  
                               where  START_DT=v_rec.taarich
                                and   BRANCH=v_rec.KOD_SNIF_TNUAA;
                              end;
                   end;
          END LOOP;
          CLOSE v_Cur;
...
EXCEPTION
      WHEN OTHERS THEN
        RAISE;
END pro_ins_amount_of_drivers_tnua;
/
