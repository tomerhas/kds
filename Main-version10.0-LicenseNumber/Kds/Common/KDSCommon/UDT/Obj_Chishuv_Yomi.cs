//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by a tool.
//     Runtime Version:2.0.50727.3053
//
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace KDSCommon.UDT
{
    using System;
    using Oracle.DataAccess.Client;
    using Oracle.DataAccess.Types;
    using System.Xml.Serialization;
    using System.Xml.Schema;
    
    
    public class OBJ_CHISHUV_YOMI : INullable, IOracleCustomType, IXmlSerializable {
        
        private bool m_IsNull;
        
        private long m_BAKASHA_ID;
        
        private bool m_BAKASHA_IDIsNull;
        
        private System.DateTime m_TKUFA;
        
        private bool m_TKUFAIsNull;
        
        private System.DateTime m_TAARICH;
        
        private bool m_TAARICHIsNull;

        private float m_ERECH_RECHIV;
        
        private bool m_ERECH_RECHIVIsNull;
        
        private int m_MISPAR_ISHI;
        
        private bool m_MISPAR_ISHIIsNull;
        
        private int m_KOD_RECHIV;
        
        private bool m_KOD_RECHIVIsNull;
        
        public OBJ_CHISHUV_YOMI() {
            // TODO : Add code to initialise the object
            this.m_BAKASHA_IDIsNull = true;
            this.m_TKUFAIsNull = true;
            this.m_TAARICHIsNull = true;
            this.m_ERECH_RECHIVIsNull = true;
            this.m_MISPAR_ISHIIsNull = true;
            this.m_KOD_RECHIVIsNull = true;
        }
        
        public OBJ_CHISHUV_YOMI(string str) {
            // TODO : Add code to initialise the object based on the given string 
        }
        
        public virtual bool IsNull {
            get {
                return this.m_IsNull;
            }
        }
        
        public static OBJ_CHISHUV_YOMI Null {
            get {
                OBJ_CHISHUV_YOMI obj = new OBJ_CHISHUV_YOMI();
                obj.m_IsNull = true;
                return obj;
            }
        }
        
        [OracleObjectMappingAttribute("BAKASHA_ID")]
        public long BAKASHA_ID {
            get {
                return this.m_BAKASHA_ID;
            }
            set {
                this.m_BAKASHA_ID = value;
                this.m_BAKASHA_IDIsNull= false;
            }
        }
        
        public bool BAKASHA_IDIsNull {
            get {
                return this.m_BAKASHA_IDIsNull;
            }
            set {
                this.m_BAKASHA_IDIsNull = value;
            }
        }
        
        [OracleObjectMappingAttribute("TKUFA")]
        public System.DateTime TKUFA {
            get {
                return this.m_TKUFA;
            }
            set {
                this.m_TKUFA = value;
                this.m_TKUFAIsNull = false;
            }
        }
        
        public bool TKUFAIsNull {
            get {
                return this.m_TKUFAIsNull;
            }
            set {
                this.m_TKUFAIsNull = value;
            }
        }
        
        [OracleObjectMappingAttribute("TAARICH")]
        public System.DateTime TAARICH {
            get {
                return this.m_TAARICH;
            }
            set {
                this.m_TAARICH = value;
                this.m_TAARICHIsNull = false;
            }
        }
        
        public bool TAARICHIsNull {
            get {
                return this.m_TAARICHIsNull;
            }
            set {
                this.m_TAARICHIsNull = value;
            }
        }
        
        [OracleObjectMappingAttribute("ERECH_RECHIV")]
        public float ERECH_RECHIV
        {
            get {
                return this.m_ERECH_RECHIV;
            }
            set {
                this.m_ERECH_RECHIV = value;
                this.m_ERECH_RECHIVIsNull = false;
            }
        }
        
        public bool ERECH_RECHIVIsNull {
            get {
                return this.m_ERECH_RECHIVIsNull;
            }
            set {
                this.m_ERECH_RECHIVIsNull = value;
            }
        }
        
        [OracleObjectMappingAttribute("MISPAR_ISHI")]
        public int MISPAR_ISHI {
            get {
                return this.m_MISPAR_ISHI;
            }
            set {
                this.m_MISPAR_ISHI = value;
                this.m_MISPAR_ISHIIsNull = false;
            }
        }
        
        public bool MISPAR_ISHIIsNull {
            get {
                return this.m_MISPAR_ISHIIsNull;
            }
            set {
                this.m_MISPAR_ISHIIsNull = value;
            }
        }
        
        [OracleObjectMappingAttribute("KOD_RECHIV")]
        public int KOD_RECHIV {
            get {
                return this.m_KOD_RECHIV;
            }
            set {
                this.m_KOD_RECHIV = value;
                this.m_KOD_RECHIVIsNull= false;
            }
        }
        
        public bool KOD_RECHIVIsNull {
            get {
                return this.m_KOD_RECHIVIsNull;
            }
            set {
                this.m_KOD_RECHIVIsNull = value;
            }
        }
        
        public virtual void FromCustomObject(Oracle.DataAccess.Client.OracleConnection con, System.IntPtr pUdt) {
            if ((BAKASHA_IDIsNull == false)) {
                Oracle.DataAccess.Types.OracleUdt.SetValue(con, pUdt, "BAKASHA_ID", this.BAKASHA_ID);
            }
            if ((TKUFAIsNull == false)) {
                Oracle.DataAccess.Types.OracleUdt.SetValue(con, pUdt, "TKUFA", this.TKUFA);
            }
            if ((TAARICHIsNull == false)) {
                Oracle.DataAccess.Types.OracleUdt.SetValue(con, pUdt, "TAARICH", this.TAARICH);
            }
            if ((ERECH_RECHIVIsNull == false)) {
                Oracle.DataAccess.Types.OracleUdt.SetValue(con, pUdt, "ERECH_RECHIV", this.ERECH_RECHIV);
            }
            if ((MISPAR_ISHIIsNull == false)) {
                Oracle.DataAccess.Types.OracleUdt.SetValue(con, pUdt, "MISPAR_ISHI", this.MISPAR_ISHI);
            }
            if ((KOD_RECHIVIsNull == false)) {
                Oracle.DataAccess.Types.OracleUdt.SetValue(con, pUdt, "KOD_RECHIV", this.KOD_RECHIV);
            }
        }
        
        public virtual void ToCustomObject(Oracle.DataAccess.Client.OracleConnection con, System.IntPtr pUdt) {
            this.BAKASHA_IDIsNull = Oracle.DataAccess.Types.OracleUdt.IsDBNull(con, pUdt, "BAKASHA_ID");
            if ((BAKASHA_IDIsNull == false)) {
                this.BAKASHA_ID = ((long)(Oracle.DataAccess.Types.OracleUdt.GetValue(con, pUdt, "BAKASHA_ID")));
            }
            this.TKUFAIsNull = Oracle.DataAccess.Types.OracleUdt.IsDBNull(con, pUdt, "TKUFA");
            if ((TKUFAIsNull == false)) {
                this.TKUFA = ((System.DateTime)(Oracle.DataAccess.Types.OracleUdt.GetValue(con, pUdt, "TKUFA")));
            }
            this.TAARICHIsNull = Oracle.DataAccess.Types.OracleUdt.IsDBNull(con, pUdt, "TAARICH");
            if ((TAARICHIsNull == false)) {
                this.TAARICH = ((System.DateTime)(Oracle.DataAccess.Types.OracleUdt.GetValue(con, pUdt, "TAARICH")));
            }
            this.ERECH_RECHIVIsNull = Oracle.DataAccess.Types.OracleUdt.IsDBNull(con, pUdt, "ERECH_RECHIV");
            if ((ERECH_RECHIVIsNull == false)) {
                this.ERECH_RECHIV = ((int)(Oracle.DataAccess.Types.OracleUdt.GetValue(con, pUdt, "ERECH_RECHIV")));
            }
            this.MISPAR_ISHIIsNull = Oracle.DataAccess.Types.OracleUdt.IsDBNull(con, pUdt, "MISPAR_ISHI");
            if ((MISPAR_ISHIIsNull == false)) {
                this.MISPAR_ISHI = ((int)(Oracle.DataAccess.Types.OracleUdt.GetValue(con, pUdt, "MISPAR_ISHI")));
            }
            this.KOD_RECHIVIsNull = Oracle.DataAccess.Types.OracleUdt.IsDBNull(con, pUdt, "KOD_RECHIV");
            if ((KOD_RECHIVIsNull == false)) {
                this.KOD_RECHIV = ((int)(Oracle.DataAccess.Types.OracleUdt.GetValue(con, pUdt, "KOD_RECHIV")));
            }
        }
        
        public virtual void ReadXml(System.Xml.XmlReader reader) {
            // TODO : Read Serialized Xml Data
        }
        
        public virtual void WriteXml(System.Xml.XmlWriter writer) {
            // TODO : Serialize object to xml data
        }
        
        public virtual XmlSchema GetSchema() {
            // TODO : Implement GetSchema
            return null;
        }
        
        public override string ToString() {
            // TODO : Return a string that represents the current object
            return "";
        }
        
        public static OBJ_CHISHUV_YOMI Parse(string str) {
            // TODO : Add code needed to parse the string and get the object represented by the string
            return new OBJ_CHISHUV_YOMI();
        }
    }
    
    // Factory to create an object for the above class
    [OracleCustomTypeMappingAttribute("KDSADMIN.OBJ_CHISHUV_YOMI")]
    public class OBJ_CHISHUV_YOMIFactory : IOracleCustomTypeFactory {
        
        public virtual IOracleCustomType CreateObject() {
            OBJ_CHISHUV_YOMI obj = new OBJ_CHISHUV_YOMI();
            return obj;
        }
    }
}
