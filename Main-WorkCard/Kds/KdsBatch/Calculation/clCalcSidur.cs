...
namespace KdsBatch
{
    class clCalcSidur : GlobalDataset
    {
        ...
        //    try
        //    {
        //        if (clCalcGeneral.objMeafyeneyOved.GetMeafyen(33).IntValue == 40)
        //        {
        ...
        public void CalcRechiv12()
        {
            ...
            try
            {
                if (_oGeneralData.objMeafyeneyOved.GetMeafyen(45).Value.Trim() != "")
                {
                    if (int.Parse(_oGeneralData.objMeafyeneyOved.GetMeafyen(45).Value) > 0)
                    {
                        ...
                        if (drSidurim[I]["ein_leshalem_tos_lila"].ToString() == "")
                        {
                            ...
                            if (fZmanLilaSidureyBoker == 0)
                            {
                                ...
                                if (!(_oGeneralData.objPirteyOved.iDirug == 85 && _oGeneralData.objPirteyOved.iDarga == 30))
                                {
                                    ...
                                    iIsuk = _oGeneralData.objPirteyOved.iIsuk;
                                    if ((iIsuk == 122 || iIsuk == 123 || iIsuk == 124 || iIsuk == 127) && iMisparSidur == 99001 && DateHelper.GetSugMishmeret(_iMisparIshi, dTaarich, clCalcData.iSugYom, dShatHatchalaLetashlum, dShatGmarLetashlum, _oGeneralData.objParameters) == enSugMishmeret.Liyla.GetHashCode())
                                    {
            ...
            try
            {
                ...
                if (fMichsaYomit > 0 && fNochehutLetashlum >= fMichsaYomit)
                {
                    ...
                    for (int I = 0; I < drSidurim.Length; I++)
                    {
                        ...
                        //if (dShatHatchalaLetashlum >= _oGeneralData.objMeafyeneyOved.ConvertMefyenShaotValid(dTaarich, _oGeneralData.objMeafyeneyOved.GetMeafyen(25).Value))
                        //{
                        ...
                        if (drSidureyGrira.Length > 0)
                        {
                            for (int J = 0; J < drSidureyGrira.Length; J++)
                            {
                                ...
                                if (drSidureyGrira[J]["mispar_sidur"].ToString().Substring(0, 2) != "99")
                                {
                                    ...
                                    if (CheckSugSidur(clGeneral.enMeafyen.SugAvoda.GetHashCode(), enSugAvoda.Grira.GetHashCode(), dTaarich, iSugSidur))
                                    {
                                        bNotCalc = true;
                                        break;
                ...
                if (_drSidurRagil.Length > 0)
                {
                    for (int I = 0; I < _drSidurRagil.Length; I++)
                    {
                        ...
                        if (bYeshSidur)
                        {
                            ...
                            iSugYomNext = DateHelper.GetSugYom(clCalcData.DtYamimMeyuchadim, DateTime.Parse(dTaarich.ToShortDateString()).AddDays(1), clCalcData.DtSugeyYamimMeyuchadim);//, _oGeneralData.objMeafyeneyOved.GetMeafyen(56).IntValue);
        ...
        public void CalcRechiv210(out float fZmanAruchatBoker, out float fZmanAruchatTzharim, out float fZmanAruchatErev)
        {
            ...
            try
            {
                if (_oGeneralData.objMeafyeneyOved.GetMeafyen(100).IntValue > 0)
                {
        ...
        public void CalcRechiv276()
        {
            ...
            try
            {
                _drSidurim = clCalcData.DtYemeyAvoda.Select("Lo_letashlum=0 and mispar_sidur is not null");
            ...
            {
                clLogBakashot.SetError(_lBakashaId, "E", null, clGeneral.enRechivim.NochechutLePremiyaMeshekGrira.GetHashCode(), _iMisparIshi, dTaarich, iMisparSidur, dShatHatchalaSidur, null, null, "CalcSidur: " + ex.Message, null);
                throw (ex);
            }
            finally
            {
        ...
        public void CalcRechiv277()
        {
            ...
            {
                clLogBakashot.SetError(_lBakashaId, "E", null, clGeneral.enRechivim.NochechutLePremiyaMeshekKonenutGrira.GetHashCode(), _iMisparIshi, dTaarich, iMisparSidur, dShatHatchalaSidur, null, null, "CalcSidur: " + ex.Message, null);
                throw (ex);
            }
        }
                ...
                if (drSidurim.Length > 0)
                {
                    for (int I = 0; I < drSidurim.Length; I++)
                    {
                        if (drSidurim[I]["ein_leshalem_tos_lila"].ToString() == "")
                        {
                            ...
                            if ((iIsuk == 122 || iIsuk == 123 || iIsuk == 124 || iIsuk == 127) && iMisparSidur == 99001 && DateHelper.GetSugMishmeret(_iMisparIshi, dTaarich, iSugYomLemichsa, dShatHatchalaSidur, DateTime.Parse(drSidurim[I]["shat_gmar_sidur"].ToString()), _oGeneralData.objParameters) == enSugMishmeret.Liyla.GetHashCode())
                            {
        ...
        private bool CheckShabat(int iMisparSidur, int iDay, int iSugYom, DateTime dShatHatchalaSidur, DateTime dShatGmarSidur, ref float fErechChol, ref float fErechShabat, bool bCheckMafil, float ZmanHafsaka)
        {
            ...
            if (bShabat)
            {
                ...
                { bNextShabat = true; }
                else { bNextShabat = clDefinitions.CheckShaaton(clCalcData.DtSugeyYamimMeyuchadim, DateHelper.GetSugYom(clCalcData.DtYamimMeyuchadim, dNextDay, clCalcData.DtSugeyYamimMeyuchadim), dNextDay); } //, _oGeneralData.objMeafyeneyOved.GetMeafyen(56).IntValue), dNextDay); }
        ...
        {
            throw new Exception("this function is not in use");
