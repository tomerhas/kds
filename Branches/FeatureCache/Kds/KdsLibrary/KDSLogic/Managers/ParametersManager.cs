    ...
    public class ParametersManager : IParametersManager
    {
        ...
        private void SetParameters(clParametersDM cls, DateTime dCardDate, int iSugYom)
        {
            ...
            try
            {
                ...
                if (cls.dSidurEndLimitHourParam3 >= clGeneral.GetDateTimeFromStringHour("00:01", dCardDate) && cls.dSidurEndLimitHourParam3 <= clGeneral.GetDateTimeFromStringHour("07:59", dCardDate))
                {
                ...
                if (cls.dSidurLimitShatGmarMafilim >= clGeneral.GetDateTimeFromStringHour("00:01", dCardDate) && cls.dSidurLimitShatGmarMafilim <= clGeneral.GetDateTimeFromStringHour("07:59", dCardDate))
                {
                ...
                cls.dSummerStart = GetParamDate(sTmp, dCardDate);// String.IsNullOrEmpty(sTmp) ? DateTime.Parse(string.Concat("01/01", "/", clGeneral.cYearNull.ToString())) : DateTime.Parse(string.Concat(sTmp, "/", dCardDate.Year.ToString()));

                //26- תאריך שמציין סיום שעון קיץ (אגד) בפורמט  DD/MM
                sTmp = GetOneParam(26, dCardDate);
                cls.dSummerEnd = GetParamDate(sTmp, dCardDate);// String.IsNullOrEmpty(sTmp) ? DateTime.Parse(string.Concat("01/01", "/", clGeneral.cYearNull.ToString())) : DateTime.Parse(string.Concat(sTmp, "/", dCardDate.Year.ToString()));
                ...
                cls.dStartNihulVShivik = GetParamDate(sTmp, dCardDate);//String.IsNullOrEmpty(sTmp) ? DateTime.Parse(string.Concat("01/01", "/", clGeneral.cYearNull.ToString())) : DateTime.Parse(string.Concat(sTmp, "/", dCardDate.Year.ToString()));

                //45 -  אירועי קיץ - תאריך חוקי לתחילת סידור תפעול,  
                sTmp = GetOneParam(45, dCardDate);
                cls.dStartTiful = GetParamDate(sTmp, dCardDate); //String.IsNullOrEmpty(sTmp) ? DateTime.Parse(string.Concat("01/01", "/", clGeneral.cYearNull.ToString())) : DateTime.Parse(string.Concat(sTmp, "/", dCardDate.Year.ToString()));

                //46 -   אירועי קיץ - תאריך חוקי לסיום סידור שיווק,ל,
                sTmp = GetOneParam(46, dCardDate);
                cls.dEndNihulVShivik = GetParamDate(sTmp, dCardDate); // String.IsNullOrEmpty(sTmp) ? DateTime.Parse(string.Concat("01/01", "/", clGeneral.cYearNull.ToString())) : DateTime.Parse(string.Concat(sTmp, "/", dCardDate.Year.ToString())); 
        ...
        private void SetShatKnisatShabat(clParametersDM cls, DateTime dTaarich, int iSugYom)
        {
            ...
            {//יום העצמאות - פרמטר 8
                cls.dKnisatShabat = clGeneral.GetDateTimeFromStringHour(GetOneParam(8, dTaarich), dTaarich);
            }
            else
            {
                //נקבע את שעת כניסת השבת לפי התקופה
                ...
                {//פרמטר 7 קיץ                        
                    cls.dKnisatShabat = clGeneral.GetDateTimeFromStringHour(GetOneParam(7, dTaarich), dTaarich);
                    // iShabatStart = iShabatStartInSummer;
                }
                else
                {//חורף פרמטר 6
                    cls.dKnisatShabat = clGeneral.GetDateTimeFromStringHour(GetOneParam(6, dTaarich), dTaarich);
                    //iShabatStart = iShabatStartInAuttom; //פרמטר 6  
                }
            }
        }
